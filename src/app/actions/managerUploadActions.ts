...
export async function getUploadPeriodsAction(
    ...
    try {
        ...
        const windows = new Map<string, { periodStart: string; periodEnd: string; open: number; total: number }>()
        for (const p of (data || [])) {
            const key = periodWindowKey(p.period_start)
            const entry = windows.get(key) || { periodStart: p.period_start, periodEnd: p.period_end, open: 0, total: 0 }
            entry.total++
            if (!p.report_id) entry.open++
...
export async function processUploadAction(input: {
    ...
    try {
        ...
        const byEmployee = new Map<string, any>(inWindow.map((p: any) => [p.employee_id, p]))
            ...
            try {
                ...
                const { error: periodErr } = await (admin as any)
                    ...
                    .update({
                        status: 'submitted',
                        report_id: reportId,
                        late_submitted: period.status === 'missed',
                    })
                    .eq('id', period.id)
